            ...
            for file in flist
                ...
                if m !== nothing
                    ...
                    jldopen(joinpath(root, file)) do f
                        ...
                        f_out["$U@$beta/$kn/chi_sp"] = f["nlQ_sp"].χ
                        f_out["$U@$beta/$kn/chi_ch"] = f["nlQ_ch"].χ
                        f_out["$U@$beta/$kn/λ_sp_old"] = f["λsp_old"]
                        #f_out["$U@$beta/$kn/λch_range"] = f["λch_range"]
                        #f_out["$U@$beta/$kn/spOfch"] = f["spOfch"]
                        #f_out["$U@$beta/$kn/λsp_of_λch_res"] = f["λsp_of_λch_res"]
                        f_out["$U@$beta/$kn/λnew"] = f["λnew"]
                        f_out["$U@$beta/$kn/log"] = f["log"]
                    end
                end
            end
        end
