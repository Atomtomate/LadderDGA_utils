...
function run_gather(prefix, out_fn)
    ...
    df = DataFrame(U=Float64[], β=Float64[], Nk=Int[], Nf=Int[], μ_DMFT=Float64[], n=Float64[], EKin_DMFT=Float64[], EPot_DMFT=Float64[], Σ_tc=String[],
                   qi_χm_max=Int[], qi_χd_max=Int[], χm_max=Float64[], χd_max=Float64[], χm_AF=Float64[], χm_FM=Float64[], 
                   qi_χm_min=Int[], qi_χd_min=Int[], χm_min=Float64[], χd_min=Float64[],
    #
                   λm_m=Float64[], λm_d=Float64[], qi_χm_λm_max=Int[], qi_χd_λm_max=Int[], μ_m=Float64[],
    ...
    #
                   λdm_m=Float64[], λdm_d=Float64[], qi_χm_λdm_max=Int[], qi_χd_λdm_max=Int[], μ_dm=Float64[], EKin_p1_dm=Float64[],EKin_p2_dm=Float64[],  EPot_p1_dm=Float64[], EPot_p2_dm=Float64[], PP_p1_dm=Float64[], PP_p2_dm=Float64[], 
    ...
    #
                   λm_sc_m=Float64[], λm_sc_d=Float64[], qi_χm_λm_sc_max=Int[], qi_χd_λm_sc_max=Int[], μ_m_sc=Float64[], EKin_p1_m_sc=Float64[],EKin_p2_m_sc=Float64[],  EPot_p1_m_sc=Float64[], EPot_p2_m_sc=Float64[], PP_p1_m_sc=Float64[], PP_p2_m_sc=Float64[], 
    ...
    #
                   λdm_sc_m=Float64[], λdm_sc_d=Float64[], qi_χm_λdm_sc_max=Int[], qi_χd_λdm_sc_max=Int[], μ_dm_sc=Float64[], EKin_p1_dm_sc=Float64[], EKin_p2_dm_sc=Float64[], EPot_p1_dm_sc=Float64[], EPot_p2_dm_sc=Float64[], PP_p1_dm_sc=Float64[], PP_p2_dm_sc=Float64[], 
        ...
        if endswith(root, "lDGA_julia")
            ...
            for ldga_fn in filelist
                ...
                row = jldopen(joinpath(root, ldga_fn), "r") do f
                    ...
                    tc_type_i = findfirst("_tc", ldga_fn)
                    tc_type = ldga_fn[first(tc_type_i)+3:first(sl)]
                    ...
                    row = [mP.U, mP.β, Nk, Nf, mP.μ, mP.n, mP.Ekin_1Pt, mP.Epot_1Pt, tc_type, qi_m_max, qi_d_max, χm_max, χd_max, 
                                χm_AF, χm_FM, qi_m_min, qi_d_min, χm_λ_min, χd_λ_min]
                    ...
                    for ri in res_names
                        row_i = if !haskey(f, ri) || isnothing(f[ri])
                            ...
                            [NaN, NaN, -1, -1, repeat([NaN], 7)..., -1, -1, NaN, NaN, -1, -1, NaN, NaN, NaN]
                        else
                            res = f[ri]
                            Σ_N, ki_N, Σ_AN, ki_AN, Σ_N_lf, ki_N_lf, Σ_AN_lf, ki_AN_lf, rc = if !isnothing(res.Σ_ladder) 
                                ...
                                Σ_N, ki_N, Σ_AN, ki_AN = find_k_N_AN(res.Σ_ladder, efmask, kG, mP)
                                Σ_N_lf, ki_N_lf, Σ_AN_lf, ki_AN_lf = find_k_N_AN(res.Σ_ladder, efmask, kG, mP)
                            ...
                            [res.λm, res.λd, qi_λ_m_max, qi_λ_d_max, res.μ, res.EKin_p1, res.EKin_p2, res.EPot_p1, res.EPot_p2, res.PP_p1, res.PP_p2, 
                                ki_N, ki_AN, Σ_N, Σ_AN, ki_N_lf, ki_AN_lf, Σ_N_lf, Σ_AN_lf, rc]
                        end
                        row = vcat(row, row_i)
                    end
