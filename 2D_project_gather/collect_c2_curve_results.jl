...
df = DataFrame(β = Float64[], U = Float64[], ndens = Float64[], μ_dmft = Float64[], μ_sc = Float64[],
...
for (root, dirs, files) in walkdir(dir)
    ...
    for file in flist
        ...
        jldopen(joinpath(root, file), "r") do f
            ...
            χd0 = real(f["χch"].data[1,151])
            χd0_m = 1/ (1 / χd0)
            χd0_dm = 1/ (1 / χd0 + f["λspch"][2])
            χd0_dm_sc = 1/ (1 / χd0 + f["λspch_sc"][2])
            χdπ = real(f["χch"].data[end,151])
            χdπ_m = 1/ (1 / χdπ)
            χdπ_dm = 1/ (1 / χdπ + f["λspch"][2])
            χdπ_dm_sc = 1/ (1 / χdπ + f["λspch_sc"][2])

            row = [beta, U, mP.n, mP.μ, f["μsc"], f["λsp_old"], f["λspch"][1], f["λspch"][2], f["λspch_sc"][1], f["λspch_sc"][2], χAF, χAF_m, χAF_dm, χAF_dm_sc, χd0, χd0_m, χd0_dm, χd0_dm_sc, χdπ, χdπ_m, χdπ_dm, χdπ_dm_sc, real(f["χ0_i_0_DMFT"]), real(f["χ0_i_π_DMFT"]), real(f["χ0_i_0"]), real(f["χ0_i_π"])]
            push!(df, row)
        end
    end
end
